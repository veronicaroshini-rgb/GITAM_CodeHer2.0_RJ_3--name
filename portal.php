...
// 8. RESET ATTENDANCE (New Feature)
...
// 10. UPDATE SEMESTER DETAILS (Settings Tab)
if ($user_id && isset($_POST['update_semester'])) {
    $s1 = !empty($_POST['sess1']) ? $_POST['sess1'] : null;
    $s2 = !empty($_POST['sess2']) ? $_POST['sess2'] : null;
    $pdo->prepare("UPDATE semesters SET name=?, start_date=?, end_date=?, sessional1_date=?, sessional2_date=? WHERE id=? AND user_id=?")
        ->execute([$_POST['name'], $_POST['start'], $_POST['end'], $s1, $s2, $_POST['sem_id'], $user_id]);
    header("Location: ?page=view&id=".$_POST['sem_id']."&tab=settings"); exit;
}

// 11. RENAME SUBJECT (Settings Tab)
if ($user_id && isset($_POST['rename_subject'])) {
    $pdo->prepare("UPDATE subjects SET name=? WHERE id=?")->execute([$_POST['sub_name'], $_POST['sub_id']]);
    header("Location: ?page=view&id=".$_POST['sem_id']."&tab=settings"); exit;
}

// 12. DELETE SUBJECT (Settings Tab)
...
// 13. ADD NEW SUBJECT (Settings Tab)
        ...
        <div class="card" style="width:100%; max-width:400px; padding:40px;">
            ...
            <div id="login-form">
                <form method="POST">
                    ...
                    <input type="password" name="password" required style="width:100%; padding:12px; margin:5px 0 20px; border:1px solid #ddd; border-radius:8px; box-sizing:border-box;">
            ...
            <?php elseif ($tab == 'analysis'): ?>
                ...
                <div class="card">
                    ...
                    <p style="color:#6b7280; font-size:0.9em; margin-bottom:25px;">
                        Total classes are <strong>calculated automatically</strong>.
                    </p>
                    ...
                    <div style="display:flex; justify-content:space-between; margin-bottom:15px; font-weight:bold; color:#9ca3af; font-size:0.85em; text-transform:uppercase;">
                        ...
                        <span style="flex:2; text-align:right;">Status (75% Req)</span>
                    </div>
                    ...
                    <?php foreach($subjects as $sub): 
                        ...
                        $msg = ($safe_skips_remaining > 0) ? "Safe: Skip $safe_skips_remaining more" : "⚠️ Attend ".abs($safe_skips_remaining);
                    ?>
                ...
                <?php 
                    ...
                    function getSessBadgeSmart($total_proj, $actual_present) {
                        if ($total_proj == 0) return ["Not Set", "#f3f4f6", "#6b7280", "Set Date"];
                        $max_absences_allowed = floor($total_proj * 0.25);
                    ...
                    list($s1_txt, $s1_bg, $s1_col, $s1_msg) = getSessBadgeSmart($s1_projected_total, $s1_stats);
                    list($s2_txt, $s2_bg, $s2_col, $s2_msg) = getSessBadgeSmart($s2_projected_total, $s2_stats);
                ?>
                <div class="card">
                    <h3>Sessional Status</h3>
                    <p style="color:#6b7280; font-size:0.9em; margin-bottom:15px;">Status is based on <b>absences</b>. You remain "Safe" as long as you don't exceed the 25% skip limit.</p>
                ...
                <div class="grid">
                    <div class="card">
                        ...
                        <form method="POST">
                            ...
                            <div style="display:flex; gap:10px; margin-bottom:15px; border-top:1px solid #eee; padding-top:15px;">
                                <div style="flex:1;">
                    ...
                    <div class="card">
                        ...
                        <form method="POST" style="display:flex; gap:10px; margin-bottom:10px; align-items:center;">
                            ...
                            <input type="text" name="sub_name" value="<?php echo htmlspecialchars($sub['name']); ?>" style="flex:1; padding:8px; border:1px solid #ddd; border-radius:6px;">
                            <button name="rename_subject" class="btn" style="padding:8px 12px; font-size:0.85em;">Rename</button>
